...
| zstdBufferPoolTrackBorrowSites    | boolean, _false_              |                       | Captures a stack trace for each zstd data buffer in use, and writes the oldest ones to the log when the pool is empty. Use this to find a transfer that does not release its buffer. This costs a stack trace on each zstd transfer.                                                                                     |
| zstdBufferPoolSlowBorrowWarnMillis | Integer, _10000_             |                       | Specifies how long a zstd transfer waits for a free data buffer before the worker writes the pool state to the log. A pool that waits without a limit has no other way to report an empty pool.                                                                                                                          |
